...
export class ReportsService {
  ...
  async getBonusEligibility(mes: number, anio: number, departamento?: string) {
    // Consulta que selecciona solo el mejor resultado para cada empleado
    // 1. Prioriza los que son ELEGIBLES
    // 2. Entre los elegibles, elige el de MONTO más alto
    // 3. Si no es elegible para ninguno, muestra el de monto más alto que perdió
    let query = `
      WITH RankedBonos AS (
    ...
    if (departamento && departamento !== 'Todos') {
      query = query.replace('WHERE br.mes = @0 AND br.anio = @1',
        `WHERE br.mes = @0 AND br.anio = @1 AND (e.departamento = @2 OR REPLACE(e.departamento, '?', 'í') = @2)`);
      params.push(departamento);
    }
